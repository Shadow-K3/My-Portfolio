  ...
  <main class="max-w-[1000px] mx-auto py-16 relative flex-1 w-full">
    ...
    <div class="mb-16">
      ...
      <h1 class="mb-4 font-mono text-4xl font-light tracking-tight text-white uppercase md:text-5xl">
        <span class="text-[#55B6C2]">/</span>{{ m.contact_title() }}
      </h1>
      ...
      <form @submit.prevent="submitForm" class="space-y-8 lg:col-span-3">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div class="flex flex-col gap-2 group">
            <label class="text-[#55B6C2] font-mono text-xs uppercase tracking-widest">{{ m.contact_form_name() }}</label>
            <input v-model="form.name" type="text" :placeholder="m.contact_form_placeholderName()" required
          ...
          <div class="flex flex-col gap-2 group">
            <label class="text-[#55B6C2] font-mono text-xs uppercase tracking-widest">{{ m.contact_form_email() }}</label>
            <input v-model="form.email" type="email" :placeholder="m.contact_form_placeholderEmail()" required
              class="bg-transparent border border-[#ABB2BF]/20 p-4 text-white outline-none focus:border-[#55B6C2] transition-colors hover:border-[#ABB2BF]/40 font-mono" />
          </div>
        </div>

        <div class="flex flex-col gap-2">
          <label class="text-[#55B6C2] font-mono text-xs uppercase tracking-widest">{{ m.contact_form_message() }}</label>
          <textarea v-model="form.message" :placeholder="m.contact_form_placeholderMsg()" rows="5" required
        ...
        <button :disabled="isSending" type="submit"
          ...
          {{ isSending ? 'EXECUTING_SEND...' : m.contact_form_btn() }}
      ...
      <div class="space-y-8 lg:col-span-2">
        <div class="border border-[#ABB2BF]/20 p-8 bg-[#282C33]/10 backdrop-blur-sm relative group overflow-hidden">
          ...
          <h3 class="flex items-center gap-3 mb-6 font-mono text-xl font-medium text-white">
            <span class="text-[#55B6C2]">></span> {{ m.contact_sidebar_title() }}
          </h3>
